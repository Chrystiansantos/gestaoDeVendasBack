...
class usuarioController {
    ...
    updateUsuario(app, req, res) {
        ...
        if (erros.length > 0) {
            ...
            var connection = app.config.dbConnection;
            var usuarioDao = new app.app.models.usuarioDAO(connection);
            var usuarioAlterado = usuarioDao.updateUsuario(dadosUsuario);
            usuarioAlterado.then((result) => { res.json(result) }).catch(err => { res.json(err) });
        }
    }
}
module.exports = () => {
